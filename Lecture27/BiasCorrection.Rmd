...
* Confidence for correlation coefficient with $\rho = 0.577$
```{r echo=TRUE}
...
abline(v = rho,col = "red",lwd = 4)
abline(v = mean(corhat),col = "blue",lwd = 4)
```
...
* Assume that there is an increasing transformation such that 
...
with $\phi$ being unknown monotone increasing transformation of $\theta$
...
* Using the jackknife and the infinitesimal jackknife for confidence intervals in random forests prediction or classification
...
$$
* Setting:
    * The likelihood $p(D|\theta)$ is hard to evaluate or expensive to compute  
    (e.g. missing normalizing constant)
    * Easy to sample form likelihood $p(D|\theta)$
    * Easy to samples from prior $p(\theta)$
* Examples:
    * Population genetics
    * Models based of differential equations (ecology, epidemiology, systems biology)
