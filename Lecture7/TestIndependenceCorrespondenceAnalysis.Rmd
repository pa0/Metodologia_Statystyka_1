---
title: "Test of Independence and Correspondence Analysis"
...
$$\operatorname{diag}(D_c)_{j} = c = O_{\cdot j}/ O_{\cdot\cdot}$$
* Notice that $c$ is the average row probability
* Or *centroid* (weighted average of frequencies)
$$c = \sum_i \frac{O_{i\cdot}}{O_{\cdot\cdot}} O_i$$

## Correspondence Analysis

* Compare $d_{\chi^2}(x,y)$ metric 
$$d_{\chi^2}(x,y) = (x-y)^T D_c^{-1} (x-y)$$
* and in $\chi^2$ statistic $x$ are rows vectors and $y$ is centroid
$$\chi^2 = O_{\cdot\cdot} \sum_i \frac{O_{i\cdot}}{O_{\cdot\cdot}} (O_i-c)^T D_c^{-1} (O_i-c)$$
* in other words: $\chi^2/O_{\cdot\cdot}$ is weighted average of $d_{\chi^2}(x,y)$ distances of the row vectors to their centroid
* or even more words: it is the Mahalanobis metric between row vectors based on their estimated covariance matrix under the homogeneity assumption
