...
$$\widehat{E}_{ij} = O_{\cdot\cdot} \frac{O_{i\cdot}}{O_{\cdot\cdot}} \frac{O_{\cdot j}}{O_{\cdot\cdot}}$$
```{r}
library(ca)
...
$$d_{\chi^2}(\boldsymbol{x},\boldsymbol{y}) = (\boldsymbol{x}-\boldsymbol{y})^T D_c^{-1} (\boldsymbol{x}-\boldsymbol{y})$$
* to $\chi^2$ statistic 
$$\chi^2 = O_{\cdot\cdot} \sum_i p_i (\boldsymbol{a_i}-\boldsymbol{c})^T D_c^{-1} (\boldsymbol{a_i}-\boldsymbol{c})$$
* $\chi^2/O_{\cdot\cdot}$ is weighted average of $d_{\chi^2}(\boldsymbol{a_i},\boldsymbol{c})$ distances of conditional row probabilities $\boldsymbol{a_i}$ to their row centroid $\boldsymbol{c}$
