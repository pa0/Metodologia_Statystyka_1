...
Correspondence analysis is useful for text analysis. For instance, to compare text from two different authors in terms of the frequency of words used in their texts. The idea: List all words in book 1 by author 1 in columns, and list all words in book 2 by author 2 in rows, count co-occurances in a contingency table. Using association mosaic plots, and correspondence analysis, to determine what categories contributed heavily to the dependence. 

Perform this type of analysis on the following two books:

```{r}
library(languageR)
topWords = function(book,wordLength=5,ntop=20) {
  book = book[sapply(book,nchar) >= wordLength]
  sort(table(book),decreasing = TRUE)[1:ntop]
}
data(through,alice,moby,oz)
topWords(alice)
topWords(moby)
```

*Hint:* Use package *vcd* and *ca*. Only use $k$ most frequent number of words that are common in both books.
...
This homework is due on Saturday, April 23rd at 1:30 pm.
