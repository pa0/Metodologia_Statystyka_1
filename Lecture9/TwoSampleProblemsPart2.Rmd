...
* Then we get statistic (ordered combined observations $Z_{(i)} \le \dots \le Z_{(n)}$)
$$J = \frac{n_1 n_2}{d} \max_{i=1,\dots,n} \{ | F_{n_1}(Z_{(i)}) - G_{n_2}(Z_{(i)}) | \}$$

## Do it Yourself Test Statistics

* Idea: Put a metric $d(\pi,\sigma)$ between permutations $\pi$ and $\sigma$
* Possible metrics:
    * Metric 1: $K(\pi,\sigma) =$ minimum number of pairwise adjeciant transposition to go from $\pi$ to $\sigma$
    * Metric 2: $R(\pi,\sigma) = \sqrt{ \sum_{i=1}^n (\pi(i)-\sigma(i))^2 }$ 
* Example for $K(\pi,\sigma)$: 
      * $\pi = \{ 1,2,3 \} \text{ and } \sigma = \{ 3,2,1 \}$
      * Move 1: \{ 2,3,1 \}
      * Move 2: \{ 2,1,3 \}
      * Move 3: \{ 1,2,3 \}
      * So $K(\pi,\sigma) = 3$
...
* Step 2: 
    ...
    * This builds and equivilant class $[\pi]$ of size $n_1!n_2!$ which is a subset of all possible permutations

## Do it Yourself Test Statistics

* Step 3: 
    * Construct extremal set $E$ containing all permutations which are most in agreament with $H_A$ and least with $H_0$
* Step 4: 
