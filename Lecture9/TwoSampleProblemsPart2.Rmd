...
* then the statistic these ranks $C = \sum_{j=1}^{n_2} R(Y_j)$
...
df = data.frame(geese=c(rep("lead-poisoned",length(lg)),rep("healthy",length(hg))),plasma=c(lg,hg))
...
* Let  Q1,...,Qn denote the placements of the $Y_j$'s in terms of the $X$-sample
...
$$ Z_{f_p} = \frac{T^+ - n_1 n_2 / 2}{\left(V_1 + V_2 + \bar{P}\bar{Q}\right)^{1/2}} $$

## Placement Test for the Behrens–Fisher Problem

Monte Carlo simulation of distibution of $Z_{f_p}$ under the null
...
Zfp = function(trial) {
  Pi = function(i) { sum(Yj[,trial] < Xi[i,trial]) }
  Qj = function(j) { sum(Xi[,trial] < Yj[j,trial]) }
  P = sapply(1:n1,Pi)
  Q = sapply(1:n2,Qj)
  Phat = mean(P)
  Qhat = mean(Q)
  ...
  Zfp = (sum(P)-sum(Q))/(2*sqrt(V1+V2+Phat*Qhat))
  Zfp
}

ZfpNull = sapply(1:nSim, Zfp)
```
...
```{r}
df = data.frame(ZfpNull=ZfpNull)
ggplot(df, aes(ZfpNull)) + geom_histogram(binwidth = 0.1) + ggtitle("Null Distribution of Zfp")
```
