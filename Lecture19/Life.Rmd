...
* All we know is that the event has not happened at the end of follow-up
...
* There are three main types of (right) censoring times:
    ...
    * Type III: the Ui’s are random variables, $\delta_i$'s are failure indicators:
    $$\delta_i = \begin{cases} 1 & \text{ if } T_i \le U_i \\ 0 & \text{ if } T_i > U_i \end{cases}$$
...
* Let $n_i = \#\text{subjects}$ at risk at the beginning of time $t(i)$ and $d_i = \#\text{events}$ occurring at time $t(i)$
...
* Now use $1 - P(T = a_i|T \ge a_i) = P(T \ge a_{i+1})$
...
* Using an estimate $d_i/r_i$ of the the Hazard function
    * $d_i$ is the number of deaths at $a_i$ and
    * $r_i$ is the number at risk at $a_i$
$$\widehat{S}(t) = \prod_{i=1}^k \left( 1-\frac{d_i}{r_i} \right)$$
...
$$P(T \ge t) = P( \text{ survive } j \text{th interval } I_j | \text{ survived to start of } I_j)$$
...
* 4 possibilities for each interval:
    ...
    3. **Death, but no censoring**: conditional probability of not surviving the interval is \# deaths $(d)$ divided by \# at risk $(r)$ at the beginning of the interval. So the con- ditional probability of surviving the interval is $1-(d/r)$
...
* We could use the same approach by grouping the event times into intervals (say, one interval for each month), and then counting up the number of deaths (events) in each to esti- mate the probability of surviving the interval (this is called the lifetable estimate)
...
* As the intervals get finer and finer, the approximations made in estimating the probabilities of getting through each inter- val become smaller and smaller, so that the estimator con- verges to the true $S(t)$
...
* An alternative Kaplan-Meier estimator invented by Efron in 1967
...
* Efron showed that this yields the Kaplan-Meier estimator

## Properties of Kaplan-Meier Estimator

* In case of no censoring (number of subjects $n$): 
$$\widehat{S}(t) = \frac{\# \text{deaths at $t$ or greater}}{n}$$
* Just like an estimated probability from a binomial distribution:
$$\widehat{S}(t) \overset{d}{\to} N(S(t),S(t)(1-S(t))/n)$$
* Much harder in the censored case
* $\widehat{S}(t)$ is approximately normal
...
$$\operatorname{Var}(\widehat{S}(t)) = \widehat{S}(t)^2 \sum_{t(i) < t} \frac{d_i}{(r_i-d_i)r_i}$$
* Think of the KM estimator as 
$$\widehat{S}(t) = \prod_{t(i) < t} (1 - \widehat{\lambda_i})$$
* With $\widehat{\lambda_i} = d_i/r_i$
* Since $\widehat{\lambda}_i$ are binomial proportions, we have
$$\widehat{S}(t) \overset{d}{\to} N(\lambda_i,\widehat{\lambda}_i(1-\widehat{\lambda}_i)/r_i)$$
* Also assume that $\widehat{\lambda}_i$ are independent (hold for large samples)
...
$$\log(\widehat{S}(t)) = \sum_{t_{(i)} < t} \log(1-\widehat{\lambda_i})$$
* And using the independence assumptions
$$\operatorname{Var}\left(\log(\widehat{S}(t))\right) = \sum_{i_{(i)}<t} \operatorname{Var}\left(\log(1-\widehat{\lambda}_i)\right)$$
* If $Z = \log(Y)$, then $Z \sim N(\log(\mu),\left(\frac{1}{\mu}\right)^2 \sigma^2)$
$$\operatorname{Var}\left(\log(\widehat{S}(t))\right) = \sum_{i_{(i)}<t} \left(\frac{1}{1-\widehat{\lambda}_i}\right)^2 \operatorname{Var}\left(\widehat{\lambda}_i\right)$$

## Properties of Kaplan-Meier Estimator

* Then replace $\operatorname{Var}(\widehat{\lambda}_i)$ with $\widehat{\lambda}_i(1-\widehat{\lambda}_i)/r_i$ and set $\widehat{\lambda}_i = d_i/r_i$
$$\operatorname{Var}\left(\log(\widehat{S}(t))\right) = \sum_{t_{(i)}<t} \frac{d_i}{(r_i-d_i)r_i}$$
