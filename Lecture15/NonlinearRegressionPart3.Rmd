...
* What about ``stat_smooth`` in package ``ggplot2``? Not sure...
* For example, the package ``simpleboot`` uses the boostrap to compute confidence bands
* There are two different way to implement the boostrap for regression problems
    * Resample rows (assume both $Y$ and $X$ are random)
    * Resample residuals (assumes that only $Y$ is random and $x$ is fixed)
