...
* Take the gradient with respect to $a$ and set to zero
...
and we can choose the bandwidth by minimizing the cross-validation
...
boneFemales = data.frame(age=bone[females,"age"],bmd=bone[females,"spnbmd"])

ggplot(boneFemales, aes(x=age, y=bmd)) + 
  geom_point() + labs(title = "Bone Mineral Density Data") + labs(x = "Age") + labs(y = "Change in BMD")
```

## Local Polynomials

```{r fig.width=10,fig.height=5}
degree0 = with(boneFemales,loess.smooth(age,bmd,degree = 0))
degree1 = with(boneFemales,loess.smooth(age,bmd,degree = 1))
degree2 = with(boneFemales,loess.smooth(age,bmd,degree = 2))
df = data.frame(age=degree0$x,degree0=degree0$y,degree1=degree1$y,degree2=degree2$y)
df = melt(df,id = "age")

ggplot(data = df,aes(x=age,y=value,group=variable,colour=variable)) + geom_line(size=1)
```
