...
* One set is collected $X_1,\dots,X_n$ is collected using simple random sampling
...
* This is valid in general, but we'll show it with assumption: perfect rankings
...
* Since $\operatorname{E}(X_{(i)}^*)$ is distributed like the $i$th order statistics for random sample of size $k$ from a continous distribution $F(x)$ and density $f(x)$ under perfect ranking (for $i = 1,\dots,k$):
...
* Indian goverment wanted to estimate dry bark and quinine content of these plants
...
* Instead of auditing all sales invoices, we can use the book value (readily available in the company's electronic ledgers) as the auxiliary ranking variable select a subset
* Then the audit is done only on this subset of invoices
* To estimate of the percentage of fraudulent sales or total amount of fraud
