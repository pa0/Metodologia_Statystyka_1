...
$$\widehat{\theta} = T(F_n)$$
* This way one can draw as many samples from $F_n$ to compute sample variability of all kinds of \textit{statistics} $s(x^*)$, e.g. the sample mean, sample median
...
* Corresponse to a multinomial distribution $m_n(k)$ of the vector $(k_1,k_2,...k_n)$ with each of the $n$ categories are equally likely $p_i=\frac{1}{n}$
...
* To form an exhaustive bootstrap distribution of statistic $T(\mathcal{C}_n)$, we need to compute  
    ...
    * associated weights by evaluating $m_n(k)$
...
* For example, if our estimate is the sample mean for $n = 3$
    * $s(\boldsymbol{x}^{*1}) = 1/n ( x_1 + x_2 + x_3)$
    * $s(\boldsymbol{x}^{*2}) = 1/n ( x_1 + x_2 + x_2)$
* To calculate $s(\boldsymbol{x}^{*1})$ from $s(\boldsymbol{x}^{*2})$
    * $s(\boldsymbol{x}^{*1}) - x_3/n + x_2/n$
...
* If new sample vector $\tilde{\boldsymbol{x}}$ satisfies $T(\tilde{\bolsymbol{x}}) \ge t$ then change is made 
