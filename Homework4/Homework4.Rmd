...
$$
where the matrix $D = (d_{ij})$ is positive definite; $|D|$ denotes its determinant; $E X_j = \mu_j$; $E(X_j−\mu_j)(X_j−\mu_j) = \sigma_{ij}$; and $(\sigma_{ij})= D^{−1}$. If $\sigma_{ij} = \sigma^2$ when $i = j$ and $\sigma_{ij} = \sigma_{12}$ when $i \ne j$, are the observations independent? Exchangeable?
...
Show that the leave-one-out cross-validation formula for kernel regression. Let $$L_{ij} = \frac{K(X_i,X_j)}{\sum_{i=1}^n } K(x_i,x_k).$$ Let $r_{(-1)}(x)$ be the kernel regression estimator obtained by leaving out $(X_i,Y_i).$

(a) First, show that $(−i)(Xi)= nj=1SijZj$ with
$$Z_j =
\begin{cases}
Y_j & i \ne j \\
...
\end{cases}$$
(b) Second, show that 
$$$\widehat{r}_n(x_i) - \widehat{r}_{(-i)} = L_{ij} (Y_i - \widehat{r}_{(-i)}(x_i))$$
Hence,
$$Y_i $$
Conclude that
$$\frac{1}{n} $$
