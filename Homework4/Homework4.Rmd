...
Show that the leave-one-out cross-validation $$\widehat{R}(h) = \frac{1}{n} \sum_{i=1}^n (Y_i - \widehat{r}_{(-i)}(x_i))^2 \hspace{1cm}\text{is equal to}\hspace{1cm} \widehat{R}(h) = \frac{1}{n} \sum_{i=1}^n \left( \frac{Y_i - \widehat{r}_n(x_i)}{1-L_{ii}} \right).$$ Let $L_{ij} = \frac{K(x_i,x_j)}{\sum_{k=1}^n K(x_i,x_k)}.$ Let $\widehat{r}_n(x)$ be the kernel regression estimator
and $\widehat{r}_{(-1)}(x)$ be the kernel regression estimator obtained by leaving out $(x_i,Y_i).$
...
\end{cases}.$$
(b) Show that $\widehat{r}_n(x_i) - \widehat{r}_{(-i)}(x_i) = L_{ij} (Y_i - \widehat{r}_{(-i)}(x_i))$. Hence, $Y_i - \widehat{r}_{(-i)}(x_i) = \frac{Y_i - \widehat{r}_{n}(x_i)}{1-L_{ii}}.$ And finally conclude that the equality holds.

## Deadline

Please email your solutions in the form of a Rmd file to Nan Bi and Lexi Guan.

This homework is due on Monday, May 9th at 1:30 pm.
