...
$$
\frac{\sqrt{|D|}}{(2\pi)^{K/2}} \exp \left( \sum \sum d_{ij} (x_i-\mu_i)(x_j-\mu_j) \right)
$$
where the matrix $D = (d_{ij})$ is positive definite; $|D|$ denotes its determinant; $\operatorname{E}(X_j) = \mu_j$; $\operatorname{E}((X_j−\mu_j)(X_j−\mu_j)) = \sigma_{ij}$; and $(\sigma_{ij})= D^{−1}$. If $\sigma_{ij} = \sigma^2$ when $i = j$ and $\sigma_{ij} = \sigma_{12}$ when $i \ne j$, are the observations independent? Exchangeable?
...
(b) For a simple regression model where the predictor is a continuous variable, write an R function which computes the bootstrap per- centile confidence interval for the slope parameter based on Theil’s estimate.
...
\begin{cases}
Y_j & i \ne j \\
\widehat{r}_{(-i)}(x_i) & i = j
\end{cases}.$$
(b) Show that $\widehat{r}_n(x_i) - \widehat{r}_{(-i)}(x) = L_{ij} (Y_i - \widehat{r}_{(-i)}(x_i))$. Hence, $Y_i - \widehat{r}_{(-i)} = \frac{Y_i - \widehat{r}_{n}(x_i)}{1-L_{ii}}.$ And finally conclude that the equality holds

## Deadline

Please email your solutions in the form of a Rmd file to Nan Bi and Lexi Guan.

This homework is due on Friday, May 6th at 1:30 pm.
