...
* Goal is to compare several discrete random variables, which have same range $\{ 1,2,\dots,c \}$
...
## Several Discrete Random Variables Example {.build}
...
## Several Discrete Random Variables Example

```{r message=FALSE, fig.width=6.5, fig.height=6.5} 
library(vcd); mosaic(ct,shade = T)
```

## Independence of Two Discrete Random Variables

* Test independence of two discrete random variables
* To discrete random variables $X$ and $Y$ with different ranges $\{1,2,\dots,r\}$ and $\{1,2,\dots,c\}$
...
TODO: some math derivations

* checkout Julie's book and [slides](http://juliejosse.com/wp-content/uploads/2016/01/CA_stf2015.pdf)
* [Susan's paper, Section 2.6](http://statweb.stanford.edu/~susan/papers/dfc.pdf)
...
* Different than the goodness of fit tests mentioned so far
* Data in the form of contingency tables different test 
* Two discrete random variable with two possible value $\{0,1\}$
* Four categorie (0,0), (0,1), (1,0), and (1,1)
$2 \times 2$ contingency table with four categories
* Probabilies of categories $p_ij$
* Hypothesis test:
$$H_0: p_{01} - p_{10}$$
$$H_A: p_{01} \ne p_{10}$$
...
* Count number of observations for each cateogory and save in contingency table
* Wald test statistic
* TODO, etc.
