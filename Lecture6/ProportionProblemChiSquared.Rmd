...
* This is an important feature of nonparametric statistics
...
* Alternatively to testing for deviation from a binomial probability model
...
* Contingency table with frequencies of criminals who committed crimes and their alcoholics status
```{r}
...
```{r echo=TRUE}
Chi2_0 = t((chifit$observed-chifit$expected)^2/chifit$expected); Chi2_0
```
...
chifit
```
* Conclusion: Conditional on the criminal not committing fraud, alcoholic status and type of crime have the same distribution
