...
* The original data has length 2820, but only the first 2048 to make it a dyadic number
...
* Then $\widehat{\theta} = \eta_S(\tilde{\theta},\lambda_v)$, then their "VisuShrink" is
$$\widehat{f}_v = W^{-1} \widehat{\theta}$$
with $W^{-1}$ being the inverse discrete wavelet transform

## Wavelet Thresholding

* TODO: general thresholding procedure
$$\widehat{f} = W^{-1} \nu(Wy,\lambda)$$

## Wavelet Thresholding

TODO: Data example with sunspots and Visushrink

## Wavelet Thresholding

TODO: How to estimate variance (in comments)
