...
* We could use use our asymptotic results on the KM estimator $\widehat{S}(t)$ from last time to devise a test equality of survivar function at some time $t$
* But taking advantage of the entire function will make more efficient use of the data
* The most common used statistics are based on nonparametric rank tests

## Log Rank Test

* Let failure time $t_1 < \dots < t_k$ over both samples
* We construct contingency tables at every event time $t_i$
* $d_{0j}$ and $d_{1j}$ are the number of deaths in group 0 and 1
* $n_{0j}$ and $n_{1j}$ are the number at risk in groups 0 and 1
$$
\begin{tabular}{c|cc|c}
\hline
Group & Failures & Survivors & Total \\
\hline
$0$ & $d_{0i}$ & $n_{0i}-d_{0j}$ & $d_{0i}$ \\
$1$ & $d_{1i}$ & $n_{1i}-d_{1i}$ & $d_{1i}$ \\
\hline
Total & $d_{i}$ & $n_{i}-d_{i}$ & $d_{i}$ \\
\hline
\end{tabular}
$$
...
* Using this expectation and variances from previous slide and summing over all timepoints $t_k$, we get the log rank statistics 
...
* So it quantifies the sums of differences in the hazard function over all $k$ time points scaled by by the fractino of failures
