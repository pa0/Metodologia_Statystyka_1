...
ThicknessAutistic = brainMatrix[copiedFilesGroup==1,]
ThicknessHealthy = brainMatrix[copiedFilesGroup==2,]
```
...
```{r}
uncorrectedPValueImage = sapply(1:dim(ThicknessAutistic)[2], 
                            function(i) wilcox.test(ThicknessAutistic[,i],ThicknessHealthy[,i],alternative = "two.sided")$p.value)
WriteImage(makeImage(mask,img),"uncorrectedPValueImage.nii.gz")
```
