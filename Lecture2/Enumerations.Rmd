...
hist(results$thetastar,breaks=100)
abline(v = theta.hat,col = "red",lwd = 4)
```
...
Inefficient enumeration because we are actually not using the gray code algorihtm for enumerations. We can use the library parallel to speedup computations. On a laptop with 4 cores this takes about 4 hours to compute.

```{r}
library(parallel)
ptm = proc.time()
enumData = mclapply(1:10000, function(i) {
